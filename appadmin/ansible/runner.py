...
from ansible_1.callback import AdHocResultCallback, CommandResultCallback, PlaybookResultCallBack
...
class PlayBookRunner:
    ...
    def __init__(self, playbook_path, inventory=None, options=None):
        ...
        self.passwords = {"passwords":'123456'}
        self.__check()
    ...
    def run(self):
        ...
        try:
            results_callback=self.results_callback.output['plays'][0]['tasks']
            status=self.results_callback.output['stats']
            results={"results":results_callback,"status":status}
            return results
        except Exception as e:
            raise AnsibleError('The hostname parameter or groups parameter in the BaseInventory '
                               'does not match the hosts parameter in the {} file.'.format(self.playbook_path))
